...
sessionMemory.userids   = new Set([...serverIds,   ...sessionMemory.userids]);
  ...
  async function runAutomatedChecks() {
    ...
    if ((username && sessionMemory.usernames && sessionMemory.usernames.has(username)) || (userid && sessionMemory.userids && sessionMemory.userids.has(userid))) { updateStatus('⚠️ 已在库中, 自动关闭', 'warn'); safeSendMessage({ action: 'user_reviewed', user: { username, userid } }); safeSendMessage({ action: 'task_completed' }); setTimeout(requestCloseTab, 1200); return; }
