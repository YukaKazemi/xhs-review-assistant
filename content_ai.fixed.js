// content_ai.js (v10.27 - 恢复并加固 storage 同步方案)
// - 【核心】遵从您的指示，完全放弃URL传参方案，恢复使用 chrome.storage 进行关键词同步。
// - 【发送逻辑恢复】在 mainAuditLoop 中，当获取到搜索词后，通过 safeSendMessage 将其发送给 background.js 进行存储。
//   这样做比内容脚本直接存储更稳定，可以防止页面跳转过程中的时序问题。
// - 【接收逻辑恢复】在 runOnPageChange 中，为个人主页增加了从 chrome.storage.local 获取关键词并填充到面板的逻辑。
// - 【架构清理】删除了所有与URL参数相关的错误代码。

(function () {
  'use strict';

  console.log('[DEBUG-JS-LOAD] content_ai.js v10.27 (storage 同步修复版) 脚本开始执行...');
  ...
  function safeSendMessage(msg){
      ...
      return new Promise((resolve)=> { if (!extAlive()) return resolve({ ok:false, err:"invalidated" }); try { chrome.runtime.sendMessage(msg, (res)=>{ const err = chrome.runtime.lastError; if (err) { console.error(`[DEBUG-JS-MSG] <- Error response from background:`, err); return resolve({ ok:false, err:err.message }); } resolve(res || { ok:true }); }); } catch(e){ console.error(`CATCH Error sending message:`, e); resolve({ ok:false, err:String(e) }); } });
  }
  ...
  async function GET_JSON(url) {
      console.log(`[DEBUG-JS-NET] -> GET_JSON: Attempting to fetch from ${url}`);
      try {
          const r = await fetch(url, { credentials: "omit" });
          if (!r.ok) { console.error(`[DEBUG-JS-NET] <- GET_JSON: Fetch failed for ${url}, status: ${r.status}`); return null; }
          const data = await r.json();
          return data;
      } catch (e) { console.error(`[DEBUG-JS-NET] <- GET_JSON: CATCH Error during fetch for ${url}:`, e); return null; }
  }

  async function postJSON(url, data) {
      console.log(`[DEBUG-JS-NET] -> postJSON: Attempting to POST to ${url}`, data);
      try {
          const res = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data || {}), credentials: 'omit' });
          if (!res.ok) { console.error(`[DEBUG-JS-NET] <- postJSON: POST failed for ${url}, status: ${res.status}`); return null; }
          return res.json().catch(() => true);
      } catch (e) { console.error(`[DEBUG-JS-NET] <- postJSON: CATCH Error during POST for ${url}:`, e); return null; }
  }
  ...
  async function markData(manualStatus, closePage = true) {
      ...
      try {
          ...
          console.log(`[DEBUG-JS-MARK] 准备保存用户:`, { username, userid, searchTerm, email, manualStatus });
          if (username || userid) safeSendMessage({ action: 'user_reviewed', user: { username, userid } });
          const payload = { username, userid, ...getStats(), url: window.location.href.split('?')[0],    
             email
, search_term: searchTerm, status: manualStatus, bio: scrapeBio(), notes: scrapeNotesByTemplate() };
      ...
      if (closePage) {
          ...
          setTimeout(requestCloseTab, 5000);
      }
  }
    ...
    if (globalAiEnabled) {
        ...
        const payload = { ...grabData(), ...getStats(), url: window.location.href.split('?')[0], 
            email: document.getElementById('xhs-email')?.value.trim() || "",
             ...
             console.log(document.getElementById('xhs-email')?.value.trim() || "");
  ...
  async function runOnPageChange() {
      ...
      pageDetectorInterval = setInterval(async () => {
          ...
          if (pageNow !== 'unknown' || attempts > maxAttempts) {
              ...
              if (pageNow === 'profile') {
                  console.log("[DEBUG-JS-KEYWORD] 个人主页加载，正在从 storage 获取关键词...");
                  ...
                  if (term) {
                      ...
                      if (panelInput) { panelInput.value = term; console.log(`[DEBUG-JS-KEYWORD] 成功获取并填充关键词: "${term}"`); }
                  } else { console.log("[DEBUG-JS-KEYWORD] storage 中未找到关键词。"); }
                  await runAutomatedChecks();
              } else if (pageNow === 'search') {
                  await syncKnownDataForHighlighting();
                  installMutationObserver();
                  // NEW: persist current search input to storage immediately and on changes
                  try {
                      const syncKeywordNow = async () => {
                          const inp = document.querySelector('#search-input') || document.querySelector('input[placeholder*="搜索"]');
                          const v = (inp && inp.value || '').trim();
                          if (v) {
                              console.log('[DEBUG-JS-KEYWORD] search page captured keyword -> saving:', v);
                              await safeSendMessage({ action: 'save_search_term', term: v });
                          }
                      };
                      ...
                      bind();
                      // in case the bar re-renders
                      setTimeout(bind, 1500);
                      setTimeout(bind, 3000);
                  } catch(e) { console.warn('[DEBUG-JS-KEYWORD] search page sync bind failed:', e); }
  ...
  if (!window.__xhs_storage_listener_bound) {
    ...
    chrome.storage.onChanged.addListener((changes, area) => {
      ...
      if (area === 'local' && changes[SEARCH_TERM_KEY]) {
        ...
        if (input) {
          input.value = val;
          console.log('[DEBUG-JS-KEYWORD] (patch) storage.onChanged -> 更新面板搜索词:', val);
        }
      }
    });
  }
} catch (e) { console.warn('[DEBUG-JS-KEYWORD] (patch) 绑定 storage 监听失败:', e); }
