...
def mark_data():
    ...
    if not final_decision or (not username and not userid): 
        return jsonify({"status": "error", "message": "缺少关键参数"}), 400with mark_data_lock:
        try:
...
@app.route("/api/known_usernames")
def api_known_usernames():
    try:
        names = list(r.smembers(USERNAMES_SET_KEY)) if 'r' in globals() and r is not None else []
        return jsonify(names)
    except Exception as e:
        print("/api/known_usernames error:", e)
        return jsonify([])


@app.route("/api/known_userids")
def api_known_userids():
    try:
        ids = [str(x) for x in list(r.smembers(USERIDS_SET_KEY))] if 'r' in globals() and r is not None else []
        return jsonify(ids)
    except Exception as e:
        print("/api/known_userids error:", e)
        return jsonify([])
...
def touch_user():
    ...
    try:
        with mark_data_lock:
            if username: r.sadd(USERNAMES_SET_KEY, username)
            if userid:   r.sadd(USERIDS_SET_KEY, str(userid))
        # enqueue to Excel writer if the codebase has a queue
        if 'job_q' in globals():
            job = {"job_id": f"touch:{userid or username}:{int(time.time()*1000)}", "username": username, "userid": userid, "url": url, "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"), "status": "touch"}
            try:
                job_q.put(job, timeout=0.1)
            except Exception as e:
                print("touch_user enqueue failed:", e)
        return jsonify({"status":"ok"})
    except Exception as e:
        print("touch_user error:", e)
