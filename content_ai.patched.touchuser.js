...
(function () {
  ...
  async function postJSON(url,obj){ try{ const r=await fetch(url,{method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify(obj)}); if (!r.ok) return { ok: false, error: `HTTP ${r.status}` }; return r.json(); } catch(e) { return { ok: false, error: String(e) }; } }
      ...
      for (const url of uniqUrls) {
          ...
          try {
              ...
              if (isRereview) {
                  ...
                  // 将搜索词编码后附加到URL上
                  if (searchTerm) urlObj.searchParams.set('search_term', searchTerm);
              }
  ...
  async function mainAuditLoop() {
      ...
      try {
          ...
          const serverNames = new Set(Array.isArray(names) ? names : []);
const serverIds   = new Set(Array.isArray(ids) ? ids : []);
sessionMemory.usernames = new Set([...serverNames, ...sessionMemory.usernames]);
sessionMemory.userids   = new Set([...serverIds,   ...sessionMemory.userids]);
          ...
          while (!auditTaskState.shouldStop) {
              ...
              const currentSearchTerm = (function(){
  const el = document.querySelector('#search-input, input[type="search"], .search-input-container input[type="text"]');
  if (el){ const v = (el.value || el.getAttribute('value') || '').trim(); if (v) return v; }
  const p = new URLSearchParams(location.search);
  let raw = (p.get('keyword') || p.get('q') || '').trim();
  // smart-decode up to 3 rounds to handle %25E6... double-encoding
  for (let i=0;i<3;i++){ try{ const dec=decodeURIComponent(raw); if (dec===raw) break; raw=dec; }catch{ break; } }
  return raw;
})();
  ...
  async function runAutomatedChecks() {
    ...
    if ((username && sessionMemory.usernames && sessionMemory.usernames.has(username)) || (userid && sessionMemory.userids && sessionMemory.userids.has(userid))) { 
    await postJSON((window.FLASK_BASE || '') + '/touch_user', {
      username,
      userid,
      url: window.location.href.split('?')[0]
    });

    updateStatus('⚠️ 已在库中, 自动关闭', 'warn'); safeSendMessage({ action: 'user_reviewed', user: { username, userid } }); safeSendMessage({ action: 'task_completed' }); setTimeout(requestCloseTab, 1200); return; }
if ((username && knownUsernames.has(username)) || (userid && knownUserids.has(userid))) {
